...
class Visualizador:
	def __init__(self, dim_x, dim_y, escala, cor_fundo):
		"""
		...
		self.svis = pygame.Surface((dim_x, dim_y))
		self.limpar()
		
	def actualizar(self, svis_base, rect_base):
		"""
		Actualizar visualização
		@param svis_base: superfície de visualização base
		@param rect_base: rectângulo base de visualização
		"""
		svis_base.blit(self.svis, rect_base)
		pygame.display.update(rect_base)	
		
	def limpar(self):
		"""
		Limpar visualizador
		"""
		self.svis.fill(self.cor_fundo)
	...
	def agente(self, pos, ang=None, col=False, carga=False):
		...
		"""
		r = round(0.4 * self.escala)
		margem = 0.1 * self.escala
		x, y = self.pvpix(pos)
		x0 = round(x + r + margem)
		y0 = round(y + r + margem)
	...
	def vazio(self, pos):
		...
		@param pos: posição do elemento
		"""
		self.rect(pos, 0, self.cor_fundo, 0)
	...
	def vector(self, pos, mod, ang, cor=(255, 255, 0), linha=1, seta=True):
		...
		"""
		x, y = self.pvpix(pos)
		xi = x + self.escala / 2.0
		yi = y + self.escala / 2.0
		PROP_VECT = 0.5 # Proporção do vector em relação à escala
		dim = PROP_VECT * mod * self.escala
	...
	def pvpix(self, pos_virt):
		...
		"""
		xv, yv = pos_virt
		x = round(xv * self.escala)
		y = round(yv * self.escala)
		return x, y
